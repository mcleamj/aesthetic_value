...
# int_only_formula <- bf(log(aesthe_survey_abund) ~ 
#                                       (1 | Country/SiteCode),
# 
#                                     family=gaussian())
# 
...
# 
# site_intercepts <- as.data.frame(coef(int_only_model)$`Country:SiteCode`) %>%
#   rownames_to_column("SiteCode")
# site_intercepts$SiteCode <- sub(".*_", "", site_intercepts$SiteCode)
# saveRDS(site_intercepts, "outputs/site_intercepts.rds")
...
sst_model_formula <- bf(log(aesthe_survey_abund) ~ sst_mean +
                          ...
                          #s(SiteLatitude, SiteLongitude, bs = "gp",m=2) +
                          (1 | Country/SiteCode),
...
gravity_model <- brm(gravity_model_formula,
                      data=standardized_data,                  #control = list(max_treedepth = 15),
                     chains=4, iter=4000, cores=ncores,
                     prior=prior)
...
posterior_summary(dag_output, probs = c(0.05,0.95))
...
DAG_summary_table <- as.data.frame(posterior_summary(dag_output, probs=c(0.10,0.90)))
...
sst_sens_formula <- bf(log(aesthe_survey_abund) ~ sst_mean +
                         as.factor(Temperature_Zone) +
                         #s(SiteLatitude, SiteLongitude, bs = "gp",m=2) +
                         (1 | Country/SiteCode),
...
ggsave(file=here::here("figures_tables/model_checks/sensitivity_tests",
                       "sst_VIF.png"), 
       device = 'png',
       dpi = 300, 
       units = 'in', 
...
sst_ridge_plot <- ggplot(sst_ridge, aes(x = draws, y = model, fill=model)) +
  geom_density_ridges() +
...
NPP_sens_formula <- bf(log(aesthe_survey_abund) ~ NPP_mean +
                         BO_nitrate +
                         BO_phosphate +
                         sst_mean +
                         wave_energy +
...
NPP_ridge_plot <- ggplot(NPP_ridge, aes(x = draws, y = model, fill=model)) +
  geom_density_ridges() +
...
DHW_sens_formula <- bf(log(aesthe_survey_abund) ~ dhw_mean +
                         sst_mean +
                         as.factor(Temperature_Zone) +
                         #s(SiteLatitude, SiteLongitude, bs = "gp",m=2) +
                         (1 | Country/SiteCode),
...
DHW_ridge_plot <- ggplot(DHW_ridge, aes(x = draws, y = model, fill=model)) +
  geom_density_ridges() +
...
HDI_sens_formula <- bf(log(aesthe_survey_abund) ~ HDI2017 +
                         sst_mean +
                         as.factor(Temperature_Zone) +
                         #s(SiteLatitude, SiteLongitude, bs = "gp",m=2) +
                         (1 | Country/SiteCode),
...
HDI_ridge_plot <- ggplot(HDI_ridge, aes(x = draws, y = model, fill=model)) +
  geom_density_ridges() +
...
sst_richness_formula <- bf(log(aesthe_survey_abund) ~ sst_mean + nb_species + 
                          abs_latitude +
                          as.factor(Temperature_Zone) +
                            #s(SiteLongitude, SiteLatitude, bs = "gp",m=2) +
                          (1 | Country/SiteCode),                     
                        family=gaussian())
...
gravity_richness_formula <- bf(log(aesthe_survey_abund) ~ gravtot2 + nb_species +
                              HDI2017 +
                              abs_latitude + # ABSOLUTE VALUE FOR LINEARITY
                              as.factor(Temperature_Zone) +
                                #s(SiteLongitude, SiteLatitude, bs = "gp",m=2) +
...
NPP_richness_formula <- bf(log(aesthe_survey_abund) ~ NPP_mean + nb_species +
                          abs_latitude +
                          BO_nitrate +
                          BO_phosphate +
                          sst_mean +
...
depth_richness_formula <- bf(log(aesthe_survey_abund) ~ Depth + nb_species +
                            as.factor(Temperature_Zone) +
                              #s(SiteLongitude, SiteLatitude, bs = "gp",m=2) +
                              (1 | Country/SiteCode),                     
                            family=gaussian())
...
fshd_richness_formula <- bf(log(aesthe_survey_abund) ~ fshD + nb_species +
                           HDI2017 +
                           #abs_latitude +
                           as.factor(Temperature_Zone) +
                             #s(SiteLongitude, SiteLatitude, bs = "gp",m=2) +
...
HDI_richness_formula <- bf(log(aesthe_survey_abund) ~ HDI2017 + nb_species +
                          abs_latitude + 
                            sst_mean +
                          as.factor(Temperature_Zone) +
                            #s(SiteLongitude, SiteLatitude, bs = "gp",m=2) +
...
MPA_richness_formula  <- bf(log(aesthe_survey_abund) ~ MPA + nb_species +
                           HDI2017 +
                           gravtot2 +
                           as.factor(Temperature_Zone) +
                             #s(SiteLongitude, SiteLatitude, bs = "gp",m=2) +
...
benthic_PC1_richness_formula <- 
  bf(log(aesthe_survey_abund) ~ PC1_imputed + nb_species +
       Depth + 
       gravtot2 +
       MPA + 
       NPP_mean +
...
benthic_PC2_richness_formula <- 
  bf(log(aesthe_survey_abund) ~ PC2_imputed + nb_species +
       Depth + 
       gravtot2 +
       MPA + 
       NPP_mean +
...
DHW_richness_formula <- bf(log(aesthe_survey_abund) ~ dhw_mean + nb_species +
                          abs_latitude +
                          sst_mean +
                          as.factor(Temperature_Zone) +
                            #s(SiteLongitude, SiteLatitude, bs = "gp",m=2) +
...
# NPP MODEL
performance::check_model(NPP_richness_model) # 
# graphics.off()
