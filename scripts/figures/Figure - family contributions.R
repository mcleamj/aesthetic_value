...
fit_list_old <- read_rds("outputs/abundance_fit_list.rds")
fit_list <- read_rds("outputs/abundance_fit_list_spatial.rds")
...
MPA_effect_summary$abs_Estimate <- abs(MPA_effect_summary$Estimate)
...
space_sequence <- seq(1,(nrow(top_MPA_families)*2),by=1)
plot_sequence <- seq(1,(nrow(top_MPA_families)*2),by=2)

graphics.off()
par(mar=c(4,12,2,4))
scatter2D(top_MPA_families$Estimate, plot_sequence, xlim=c(min(top_MPA_families$Q10,na.rm = TRUE),max(top_MPA_families$Q90,na.rm = TRUE)),
          ylim=c(min(plot_sequence-0.25),max(plot_sequence+0.25)),cex=0,
          xlab="MPA Effect Size", ylab=NA, yaxt = "n",
          cex.lab=1.25, 
          colvar = top_MPA_families$aesthe_score,
          col=jet(n=nrow(top_MPA_families)))
...
points(top_MPA_families$Estimate, plot_sequence,pch=21,col=1,bg=plot_colors, cex=1.5)
abline(v=0, lwd=1.5, col=adjustcolor("black",alpha.f = 0.75), lty=2)

axis(2, at = plot_sequence, 
     labels = top_MPA_families$family,
     las=2, cex.axis=0.9)

title("Family Contributions to MPA Effect",
      line=1, font.main=1, cex.main=1.2)
